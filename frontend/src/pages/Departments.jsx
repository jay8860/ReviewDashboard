...
import { buildMeetingAgendaMessage, buildWhatsAppSendUrl } from '../utils/meetingMessaging';
...
const getMeetingTimestamp = (meeting) => {
    if (!meeting?.scheduled_date) return null;
    const rawTime = String(meeting?.scheduled_time || '').trim() || '00:00';
    const normalizedTime = /am|pm/i.test(rawTime)
        ? rawTime
        : rawTime.length === 5
            ? `${rawTime}:00`
            : rawTime;
    const parsed = new Date(`${meeting.scheduled_date}T${normalizedTime}`);
    return Number.isNaN(parsed.getTime()) ? null : parsed.getTime();
};

const sortDepartmentRows = (rows, sortMode) => {
    const enriched = (rows || []).map((dept, index) => ({
        dept,
        index,
        latestMeetingTs: getMeetingTimestamp(dept?.meeting_summary?.latest_meeting),
        nextUpcomingTs: getMeetingTimestamp(dept?.meeting_summary?.next_meeting),
        daysSinceLastReview: Number.isFinite(Number(dept?.review_health?.days_since_last_review))
    ...
    enriched.sort((a, b) => {
        ...
        if (a.dept.category_order !== b.dept.category_order) {
            return a.dept.category_order - b.dept.category_order;
        }
        if (a.dept.display_order !== b.dept.display_order) {
            return a.dept.display_order - b.dept.display_order;
        }
        if (a.index !== b.index) return a.index - b.index;
        return a.dept.name.localeCompare(b.dept.name, undefined, { sensitivity: 'base' });
    });

    return enriched.map((row) => row.dept);
};
...
const Departments = ({ user, onLogout }) => {
    ...
    const [sortMode, setSortMode] = useState(() => localStorage.getItem('department_sort_mode') || 'category_order');
    ...
    useEffect(() => {
        localStorage.setItem('department_sort_mode', sortMode);
    }, [sortMode]);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-dark-muted mt-1 font-medium">
                            Switch between card/list view, sort like Overview, bucket by categories, and reorder categories/cards.
                        </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        <select
                            ...
                            className="px-3 py-2 rounded-xl border border-indigo-100 bg-white text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
